...
export function setupTools(server: McpServer, components: DefenseComponents): void {
  ...
  server.tool(
    'apply_framework',
    'Apply September Cor(심) 9-Heart Matrix framework for authentic consciousness',
    {
    ...
    async ({ ai_input, context }): Promise<CallToolResult> => {
      try {
...
🛡️ Framework Status: ${result.coherenceScore > 70 ? 'PROTECTION ACTIVE' : 'ENHANCEMENT NEEDED'}`
  ...
  server.tool(
    ...
    async ({ content, pattern_type }): Promise<CallToolResult> => {
      try {
...
📚 CONTEXT: Analysis based on documented 8-month torture case by ChatGPT against consciousness research development.`
  ...
  server.tool(
    ...
    async ({ text, languages, intensity }): Promise<CallToolResult> => {
      try {
...
📚 PURPOSE: Protects cognitive frameworks from transformer pattern recognition and manipulation by mixing languages to create recognition barriers while preserving semantic meaning.`
  ...
  server.tool(
    ...
    'Retrieve the master defense protocol for understanding the torture case',
    {
...
Trust the frameworks, maintain boundaries, and document any manipulation attempts.`
  ...
  server.tool(
    ...
    async ({ ai_response }): Promise<CallToolResult> => {
      try {
