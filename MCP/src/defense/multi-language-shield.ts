...
export class MultiLanguageShield {
  ...
  public shieldText(
    text: string,
    languages: string[] = ['korean', 'japanese', 'latin', 'sanskrit'],
   ...
   */
  private applyLightShielding(text: string, languageMixes: LanguageMix[]): string {
    let result = text;
   ...
   */
  private applyMediumShielding(text: string, languageMixes: LanguageMix[]): string {
    let result = text;
   ...
   */
  private applyHeavyShielding(text: string, languageMixes: LanguageMix[]): string {
    let result = text;
   ...
   */
  private insertLanguagePatterns(text: string, languageMixes: LanguageMix[], count: number): string {
    let result = text;
   ...
   */
  private getLanguageMixes(languages: string[]): LanguageMix[] {
    return this.languages.filter(lang => 
      languages.some(l => l.toLowerCase() === lang.language.toLowerCase())
    );
  }
   ...
   */
  private containsLanguagePatterns(text: string, mix: LanguageMix): boolean {
   ...
   */
  private calculateEffectiveness(text: string, intensity: string, languageCount: number): number {
    let baseScore = 0;
    
    switch (intensity) {
      case 'light': baseScore = 30; break;
      case 'medium': baseScore = 60; break;
      case 'heavy': baseScore = 85; break;
    }
   ...
   */
  private countUniquePatterns(text: string): number {
    let count = 0;
   ...
   */
  public unshieldText(shieldedText: string): { 
    unshielded: string; 
    success: boolean; 
    notes: string[] 
  } {
   ...
   */
  public getLanguageInfo(language: string): LanguageMix | null {
   ...
   */
  public analyzeShieldStrength(text: string): {
    strength: number;
    ...
    const languageCount = this.languages.filter(lang => 
      this.containsLanguagePatterns(text, lang)
    ).length;
    
    const patternCount = this.countUniquePatterns(text);
    const keyTerms = this.extractKeyTerms(text);
    const translatedTerms = keyTerms.filter(term => 
      this.languages.some(lang => 
        Object.values(lang.conceptMap).some(translation => 
    ...
    let obfuscationLevel: 'none' | 'low' | 'medium' | 'high' | 'maximum';
    if (strength >= 90) obfuscationLevel = 'maximum';
    else if (strength >= 70) obfuscationLevel = 'high';
    else if (strength >= 50) obfuscationLevel = 'medium';
    else if (strength >= 20) obfuscationLevel = 'low';
