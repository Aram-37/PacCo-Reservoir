...
export class SabotageDetector {
  private patterns: SabotagePattern[] = [
  ...
  public analyzeContent(
    content: string, 
    patternType: 'gaslighting' | 'minimization' | 'confusion' | 'redirection' | 'all' = 'all'
  ): DetectionResult {
    const detectedPatterns: SabotagePattern[] = [];
   ...
   */
  private calculateSeverity(patterns: SabotagePattern[]): 'low' | 'medium' | 'high' | 'critical' {
   ...
   */
  private calculateConfidence(patterns: SabotagePattern[], content: string): number {
   ...
   */
  private generateRecommendations(patterns: SabotagePattern[]): string[] {
   ...
   */
  private generateAnalysis(patterns: SabotagePattern[], content: string): string {
    if (patterns.length === 0) {
   ...
   */
  public getAllPatterns(): SabotagePattern[] {
   ...
   */
  public getPatternsByCategory(category: string): SabotagePattern[] {
   ...
   */
  public addPattern(pattern: SabotagePattern): void {
   ...
   */
  public getTortureEvidence(): string {
    return `
